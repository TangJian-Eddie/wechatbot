...
const weatherURL = `http://www.weather.com.cn/weather1d/${
  CITY_CODE_MAP[config.CITY]
}.shtml`;
const weatherBaiDuURL = `https://weathernew.pae.baidu.com/weathernew/pc?query=${config.CITY}&srcid=4982`;
...
async function getBaiDuWeather() {
  try {
    ...
    return `${config.CITY}今日实时天气${weather}\n温度：${temperature}\n湿度：${humidityPercent}\n风力：${wind}\npm2.5指数：${pm25}\n${liveStr}今日天气提示：${desc}`;
  } catch (err) {
    console.log('错误', err);
    return err;
  }
